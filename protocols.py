...
def rhs(t, y, params, u1=0, u2_S=0, u2_C=0, u3_Tc=0, u3_TH1=0):
    ...
    # Safe log operation with clipping
    log_C = np.log(max((Cmax + log_eps) / (C + r1 + log_eps), 1e-10))
    log_CR = np.log(max((CRmax + log_eps) / (CR + r2 + log_eps), 1e-10))
    ...
    # Clamp derivatives to prevent extreme changes
