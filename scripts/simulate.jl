...
function parse_commandline()
    ...
    @add_arg_table! s begin
        ...
        "--settle", "-s"
            ...
            action = :store_true
        "--x0"
            help = "initial conditions [x0[i] for i in list of state variables]"
            nargs = 13
            arg_type = Float64
            default = [1.0, 0, 0, 0, 85000, 15000, 71000, 12000, 56000, 8000, 0.0085, 0.12, 0.0094]
        "--p"
            help = "predefined parameter set name (default)"
            arg_type = String
            default = "default" 
        "--output", "-o"
...
function save_solution(sol, p, filename::String)
    ...
    open(metadata_file, "w") do f
        ...
        write(f, "\nInitial conditions:\n")
        write(f, "  x₀ (prey) = $(sol[1, 1])\n")
        write(f, "  y₀ (predator) = $(sol[2, 1])\n")
        write(f, "\nFinal state:\n")
        write(f, "  x_final = $(sol[1, end])\n")
        write(f, "  y_final = $(sol[2, end])\n")
    end
    return filepath
end
...
function main()
    ...
    if args["verbose"]
        ...
        println("  Final state: x = $(sol[1, end]), y = $(sol[2, end])")
